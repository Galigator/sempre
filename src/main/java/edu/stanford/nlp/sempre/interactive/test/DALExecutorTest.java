...
public class DALExecutorTest
{
	...
	protected static void runFormula(final DALExecutor executor, final String formula, final ContextValue context, final Predicate<World> checker)
	{
	...
	private static ContextValue getContext(final String blocks)
	{
		...
		final String strigify2 = Json.writeValueAsStringHard(blocks); // some parsing
		// issue inside
		// lisptree parser
		return ContextValue.fromString(String.format("(context (graph NaiveKnowledgeGraph ((string \"%s\") (name b) (name c))))", strigify2));
	}

	public Predicate<World> selectedSize(final int n)
	{
		return x ->
		{
	...
	public void testJoin()
	{
		...
		runFormula(executor, "(: select (color ((reverse color) (color ((reverse color) (color red))))))", context, x -> x.selected().iterator().next().get("color").equals("red"));
	...
	{
		final String defaultBlocks = "[[1,1,1,\"Green\",[\"S\"]],[1,2,1,\"Blue\",[\"S\"]],[2,2,1,\"Red\",[\"S\"]],[2,2,2,\"Yellow\",[]]]";
		final ContextValue context = getContext(defaultBlocks);
	...
	{
		{
			final String defaultBlocks = "[[1,1,1,\"Green\",[]],[1,2,1,\"Blue\",[]],[2,2,1,\"Red\",[]],[2,2,2,\"Yellow\",[]]]";
			final ContextValue context = getContext(defaultBlocks);
	...
	{
		final String defaultBlocks = "[[1,1,1,\"Green\",[]],[1,2,1,\"Blue\",[]],[2,2,1,\"Red\",[]],[2,2,3,\"Yellow\",[]]]";
		final ContextValue context = getContext(defaultBlocks);
		...
		runFormula(executor, "(:foreach (or (color red) (color orange)) (:loop (number 5) (: add red top)))", context, x -> x.allItems.size() == 9);
		runFormula(executor, "(:for (or (color red) (color blue)) (:loop (number 5) (:s (: move left) (: move right) (: move left))))", context, null);

		LogInfo.end_track();
	}

	private Set<Item> real(final Set<Item> all)
	{
	...
	public void testRemove()
	{
		...
		final String defaultBlocks = "[[1,1,1,\"Green\",[]],[1,1,2,\"Green\",[\"S\"]],[1,1,3,\"Red\",[\"S\"]],[1,1,4,\"Green\",[]]]";
		final ContextValue context = getContext(defaultBlocks);
		...
		runFormula(executor, "(:for (color green) (: remove))", context, x -> x.selected.size() == 2 && real(x.allItems).size() == 1);
	...
	{
		// this is a green stick
		final String defaultBlocks = "[[1,1,1,\"Green\",[]],[1,1,2,\"Green\",[]],[1,1,3,\"Green\",[]],[1,1,4,\"Green\",[]]]";
		final ContextValue context = getContext(defaultBlocks);
		LogInfo.begin_track("testMoreActions");
		runFormula(executor, "(:s (: select *) (:for (call veryx left this) (: remove)))", context, x -> x.allItems.stream().allMatch(c -> ((Voxel) c).color.equals(Color.Fake)));
		runFormula(executor, "(:for * (:for (call veryx bot) (:loop (number 2) (:s (: add red left) (: select (call adj top))))))", context, x -> x.allItems.size() == 6);
	...
	{
		// this is a green stick
		final String defaultBlocks = "[[1,1,1,\"Green\",[\"S\"]],[1,1,2,\"Green\",[]],[1,1,3,\"Green\",[]],[1,1,4,\"Green\",[]]]";
		final ContextValue context = getContext(defaultBlocks);
	...
	{
		// this is a green stick
		final String defaultBlocks = "[[1,1,0,\"Fake\",[\"S\"]]]";
		final ContextValue context = getContext(defaultBlocks);
	...
	{
		final String defaultBlocks = "[[1,1,1,\"Green\",[\"S\"]],[1,1,2,\"Red\",[]],[1,1,3,\"Green\",[]],[1,1,4,\"Green\",[]]]";
		final ContextValue context = getContext(defaultBlocks);
	...
	public void testScoping()
	{
		...
		final ContextValue context = getContext(defaultBlocks);
		LogInfo.begin_track("testIsolation");
		runFormula(executor, "(:blk (: select (call veryx top this)))", context, x -> (Integer) x.selected.iterator().next().get("height") == 1);
		runFormula(executor, "(:blkr (: select (call adj top this)))", context, x -> (Integer) x.selected.iterator().next().get("height") == 2);

		runFormula(executor, "(:s (:blk (: add red here ) (: select (call adj top this)) (: add red here )) (: select (color red)))", context, x -> x.selected.size() == 2);
		runFormula(executor, "(:s (:blkr (: add red here ) (: select (call adj top this)) (: add red here )) (: select (color red)))", context, x -> x.selected.size() == 2);
		LogInfo.end_track();
	}
