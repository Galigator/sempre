...
/**
 * Feature computer for the the dependency-based action language TODOs: - control what categories to abstract out - efficiency improvement, right now use all
 * members of the cross product
 * 
 * @author sidaw
 */
public class DALFeatureComputer implements FeatureComputer
{
	...
	private List<String> abstractAnchors(final Derivation deriv, final List<String> tokens, final int window)
	{
		if (deriv.start == -1)
			return tokens;
		final List<String> newTokens = new ArrayList<>();
		final int startInd = Math.max(0, deriv.start - window);
		final int endInd = Math.min(tokens.size(), deriv.end + window);
	...
	private List<String> getAllNgrams(final List<String> tokens, final int n, final Derivation deriv)
	{
		final List<String> ngrams = new ArrayList<>();
		final List<String> paddedTokens = new ArrayList<>();
		...
		{
			final List<String> current = new ArrayList<>(paddedTokens.subList(i, i + n));
			ngrams.add(current.toString());
		}
		return ngrams;
	}

	private List<String> getAllSkipGrams(final List<String> tokens, final Derivation deriv)
	{
		final List<String> ngrams = new ArrayList<>();
		final List<String> paddedTokens = new ArrayList<>();
		if (tokens.size() < 3)
	...
	private void addWindowFeatures(final Example ex, final Derivation deriv)
	{
	...
	private void addStatsFeatures(final Example ex, final Derivation deriv)
	{
	...
	private void addSocialFeatures(final Example ex, final Derivation deriv)
	{
	...
	private void extractRuleFeatures(final Example ex, final Derivation deriv)
	{
	...
	private void extractSpanFeatures(final Example ex, final Derivation deriv)
	{
