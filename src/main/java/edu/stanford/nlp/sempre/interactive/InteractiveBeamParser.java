...
public class InteractiveBeamParser extends Parser
{
	...
	public InteractiveBeamParser(final Spec spec)
	{
		super(spec);
	...
	public InteractiveBeamParserState parseWithoutExecuting(final Params params, final Example ex, final boolean computeExpectedCounts)
	{
		// Parse
		final StopWatch watch = new StopWatch();
		watch.start();
		final InteractiveBeamParserState state = new InteractiveBeamParserState(this, params, ex);
		state.infer();
		watch.stop();
	...
	public ParserState newParserState(final Params params, final Example ex, final boolean computeExpectedCounts)
	{
		...
		if (Parser.opts.coarsePrune)
		{
			...
			coarseState = new InteractiveBeamParserState(this, params, ex, computeExpectedCounts, InteractiveBeamParserState.Mode.bool, null);
			coarseState.infer();
			coarseState.keepTopDownReachable();
			LogInfo.end_track();
		}
		return new InteractiveBeamParserState(this, params, ex, computeExpectedCounts, InteractiveBeamParserState.Mode.full, coarseState);
	}
}

/**
 * Stores BeamFloatingParser information about parsing a particular example. The actual parsing code lives here.
 *
 * @author Percy Liang
 * @author Roy Frostig
 * @author sidaw
...
class InteractiveBeamParserState extends ChartParserState
{
	...
	public InteractiveBeamParserState(final InteractiveBeamParser parser, final Params params, final Example ex)
	{
	...
	public InteractiveBeamParserState(final InteractiveBeamParser parser, final Params params, final Example ex, final boolean computeExpectedCounts, final Mode mode, final InteractiveBeamParserState coarseState)
	{
	...
	public void infer()
	{
		...
		if (parseFloat)
		{
			 ...
			 * rules
			 */
			final List<Rule> nothingRules = new ArrayList<>();
			...
			for (int i = 0; i < numTokens; i++)
				for (final Rule rule : nothingRules)
			...
			if (rootDerivs == null)
				rootDerivs = new ArrayList<>(Derivation.emptyList);

			final List<Derivation> actionDerivs = new ArrayList<>(Derivation.emptyList);
			if (actionDerivs != null)
			{
				final Set<Formula> formulas = new HashSet<>();
				for (final Derivation d : rootDerivs)
				{
					final Formula f = d.getFormula();
	...
	{
		final List<Derivation> chartList = Lists.newArrayList();
		for (int len = 1; len <= numTokens; ++len)
			for (int i = 0; i + len <= numTokens; ++i)
	...
	private boolean canBeRoot(final int start, final int end)
	{
	...
	private void applyNonCatUnaryRules(final int start, final int end, final int i, final Trie node, final ArrayList<Derivation> children, final IntRef numNew)
	{
		...
		if (i == end)
		{
			final Iterator<Rule> ruleIterator = node.rules.iterator();
	...
	protected void buildFloating(final int start, final int end)
	{
		for (final Rule rule : parser.grammar.getRules())
		{
			...
			if (rule.rhs.size() == 1)
			{
				...
				final String rhsCat = rule.rhs.get(0);
				final List<Derivation> derivs = chart[start][end].get(rhsCat);
	...
	protected int applyFloatingRule(final Rule rule, final int start, final int end, final Map<String, List<Derivation>> first, final Map<String, List<Derivation>> second)
	{
		final List<Derivation> derivs1 = first.get(rule.rhs.get(0));
		final List<Derivation> derivs2 = second.get(rule.rhs.get(1));
		...
		for (final Derivation deriv1 : derivs1)
			for (final Derivation deriv2 : derivs2)
			{
	...
	protected void copyDerivs(final Map<String, List<Derivation>> source, final Map<String, List<Derivation>> dest)
	{
		if (source == null || dest == null)
			return;

		for (final String cat : source.keySet())
		{
			...
			for (final Derivation deriv : source.get(cat))
	...
	protected void addDerivs(final List<Derivation> source, final List<Derivation> dest)
	{
