...
public class GrammarInducer
{
	public static class Options
	{
		...
		public Set<String> filteredCats = new HashSet<>();
	...
	public GrammarInducer(final List<String> headTokens, final Derivation def, final List<Derivation> chartList)
	{
		...
		final int numTokens = headTokens.size();
		...
		{
			final List<Derivation> filteredMatches = matches.stream().filter(d ->
			{
				return opts.simpleCats.contains(d.cat) && d.allAnchored() && d.end - d.start == 1;
			}).collect(Collectors.toList());

			final List<Derivation> packing = new ArrayList<>();
			for (int i = 0; i <= headTokens.size(); i++)
				for (final Derivation d : filteredMatches)
					if (d.start == i)
					{
						packing.add(d);
						break;
			...
			final HashMap<String, String> formulaToCat = new HashMap<>();
			packing.forEach(d -> formulaToCat.put(catFormulaKey(d), varName(d)));
			buildFormula(def, formulaToCat);
			final List<Rule> simpleInduced = induceRules(packing, def);
			for (final Rule rule : simpleInduced)
			{
			...
			if (opts.verbose > 1)
			{
				...
				matches.forEach(d -> LogInfo.logs("%f: %s\t %s", d.getScore(), d.formula, d.allAnchored()));
		...
		{
			final List<Derivation> bestPacking = bestPackingDP(matches, numTokens);

			final HashMap<String, String> formulaToCat = new HashMap<>();
			bestPacking.forEach(d -> formulaToCat.put(catFormulaKey(d), varName(d)));
			buildFormula(def, formulaToCat);
			for (final Rule rule : induceRules(bestPacking, def))
			{
			...
			if (opts.verbose > 1)
			{
				LogInfo.logs("chartList.size = %d", chartList.size());
				LogInfo.log("Potential packings: ");
				matches.forEach(d -> LogInfo.logs("%f: %s\t", d.getScore(), d.formula));
	...
	private void filterRule(final Rule rule)
	{
		if (rule.isCatUnary())
		{
	...
	static Map<String, List<Derivation>> makeChartMap(final List<Derivation> chartList)
	{
		final Map<String, List<Derivation>> chartMap = new HashMap<>();
		for (final Derivation d : chartList)
		{
	...
	static String catFormulaKey(final Derivation d)
	{
	...
	static private String getNormalCat(final Derivation def)
	{
		// return def.cat;
		final String cat = def.getCat();
		if (cat.endsWith("s"))
	...
	private void addMatches(final Derivation deriv, final Map<String, List<Derivation>> chartMap)
	{
		final String key = catFormulaKey(deriv);
	...
	class Packing
	{
		...
		{
			return score + ": " + packing.toString();
		}
	}

	// the maximum starting index of every match that ends on or before end
	private int blockingIndex(final List<Derivation> matches, final int end)
	{
		return matches.stream().filter(d -> d.end <= end).map(d -> d.start).max((s1, s2) -> s1.compareTo(s2)).orElse(Integer.MAX_VALUE / 2);
	}

	// start inclusive, end exclusive
	private List<Derivation> bestPackingDP(final List<Derivation> matches, final int length)
	{
		final List<Packing> bestEndsAtI = new ArrayList<>(length + 1);
		final List<Packing> maximalAtI = new ArrayList<>(length + 1);
		...
		final List<Derivation>[] endsAtI = new ArrayList[length + 1];

		for (final Derivation d : matches)
		{
		...
		for (int i = 1; i <= length; i++)
		{
			...
			if (endsAtI[i] != null)
			{
				for (final Derivation d : endsAtI[i])
				{
					final double score = d.getScore() + maximalAtI.get(d.start).score;
				...
				final List<Derivation> bestpacking = new ArrayList<>(maximalAtI.get(bestDerivI.start).packing);
	...
	private List<Rule> induceRules(final List<Derivation> packings, final Derivation defDeriv)
	{
		final List<String> RHS = getRHS(defDeriv, packings);
		final SemanticFn sem = getSemantics(defDeriv, packings);
		final String cat = getNormalCat(defDeriv);
		final Rule inducedRule = new Rule(cat, RHS, sem);
	...
	private void buildFormula(final Derivation deriv, final Map<String, String> replaceMap)
	{
		...
		// c.grammarInfo.end);
		final Rule rule = deriv.rule;
		final List<Derivation> args = deriv.children;
	...
	private Formula renameBoundVars(final Formula formula, final Set<String> boundvars)
	{
		if (formula instanceof LambdaFormula)
		{
			final LambdaFormula f = (LambdaFormula) formula;
			boundvars.add(f.var);
			return new LambdaFormula(newName(f.var), renameBoundVars(f.body, boundvars));
		}
		else
		{
			final Formula after = formula.map(formula1 ->
			{
	...
	private SemanticFn getSemantics(final Derivation def, final List<Derivation> packings)
	{
		...
		if (packings.size() == 0)
		{
			final SemanticFn constantFn = new ConstantFn();
		...
		final SemanticFn applyFn = new ApplyFn();
		final LispTree newTree = LispTree.proto.newList();
	...
	private List<String> getRHS(final Derivation def, final List<Derivation> packings)
	{
		final List<String> rhs = new ArrayList<>(headTokens);
		for (final Derivation deriv : packings)
		{
	...
	public static enum ParseStatus
	{
		...
		public static ParseStatus fromString(final String status)
		{
			for (final ParseStatus c : ParseStatus.values())
	...
	public static ParseStatus getParseStatus(final Example ex)
	{
		return getParseStatus(ex.predDerivations);
	}

	public static ParseStatus getParseStatus(final List<Derivation> derivs)
	{
		if (derivs.size() > 0)
		{
			for (final Derivation deriv : derivs)
