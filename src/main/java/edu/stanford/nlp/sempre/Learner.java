...
/**
 * The main learning loop. Goes over a dataset multiple times, calling the parser and updating parameters.
 *
 * @author Percy Liang
 */
public class Learner
{
	...
	public Learner(final Parser parser_, final Params params_, final Dataset dataset_)
	{
		...
		{
			final SemanticFn currSemFn = rule.getSem();
			boolean toAdd = true;
			for (final SemanticFn semFuncToUpdate : semFuncsToUpdate)
	...
	public void learn(final int numIters, final Map<String, List<Evaluation>> evaluations)
	{
		...
		for (int iter = 0; iter <= numIters; iter++)
		{
			...
			final Map<String, Evaluation> meanEvaluations = Maps.newHashMap();
			...
			for (final String group : dataset.groups())
			{
				...
				final Evaluation eval = processExamples(iter, group, dataset.examples(group), updateWeights);
	...
	private void writeParams(final int iter)
	{
		final String path = Execution.getFile("params." + iter);
		if (path != null)
		{
			params.write(path);
	...
	public void onlineLearnExample(final Example ex)
	{
	...
	public void onlineLearnExampleByFormula(final Example ex, final List<Formula> formulas)
	{
		final HashMap<String, Double> counts = new HashMap<>();
		for (final Derivation deriv : ex.predDerivations)
	...
	private Evaluation processExamples(final int iter, final String group, final List<Example> examples, final boolean computeExpectedCounts)
	{
		...
		{
			// Parallelize!
			final Parallelizer<Example> paral = new Parallelizer<>(opts.numParallelThreads);
			final LearnerParallelProcessor processor = new LearnerParallelProcessor(parser, params, prefix, computeExpectedCounts, evaluation);
		...
		else
		{
			...
			for (int e = 0; e < examples.size(); e++)
			{
				...
				final ParserState state = parseExample(params, ex, computeExpectedCounts);
	...
	private void checkGradient(final Example ex, final ParserState state)
	{
		final double eps = 1e-2;
		for (final String feature : state.expectedCounts.keySet())
		{
			LogInfo.begin_track("feature=%s", feature);
			final double computedGradient = state.expectedCounts.get(feature);
			final Params perturbedParams = params.copyParams();
			perturbedParams.getWeights().put(feature, perturbedParams.getWeight(feature) + eps);
			final ParserState perturbedState = parseExample(perturbedParams, ex, true);
			final double checkedGradient = (perturbedState.objectiveValue - state.objectiveValue) / eps;
			LogInfo.logs("Learner.checkGradient(): weight=%s, pertWeight=%s, obj=%s, pertObj=%s, feature=%s, computed=%s, checked=%s, diff=%s", params.getWeight(feature), perturbedParams.getWeight(feature), state.objectiveValue, perturbedState.objectiveValue, feature, computedGradient, checkedGradient, Math.abs(checkedGradient - computedGradient));
			LogInfo.end_track();
		}
	}
	...
	{
		for (final SemanticFn semFn : semFuncsToUpdate)
			semFn.sortOnFeedback(parser.getSearchParams(params));
	}

	private void addFeedback(final Example ex)
	{
		for (final SemanticFn semFn : semFuncsToUpdate)
	...
	private void updateWeights(final Map<String, Double> counts)
	{
	...
	private void logEvaluationStats(final Evaluation evaluation, final String prefix)
	{
	...
	private void printLearnerEventsIter(final Example ex, final int iter, final String group)
	{
		if (eventsOut == null)
			return;
		...
		if (opts.dumpFeaturesAndCompatibility)
			for (final Derivation deriv : ex.predDerivations)
			{
				...
				for (final String f : features.keySet())
				{
					final double v = features.get(f);
					fields.add(f + "=" + v);
				}
	...
	private void printLearnerEventsSummary(final Evaluation evaluation, final int iter, final String group)
	{
		if (eventsOut == null)
			return;
