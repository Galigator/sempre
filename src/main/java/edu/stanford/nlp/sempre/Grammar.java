 ...
 * supported to make life easier. All grammar-file variables should start with '@' (both loop variables and global definitions). Includes (reads in commands
 * from the given file): (include base.grammar) Control flow: (when (and tag1 (not tag2) (or tag3 tag4)) ...) Macro definitions: (def @type fb:type.object.type)
 * Iteration: (for @x (a an the) (rule $DT (@x) (IdentityFn)))
 *
 * @author Percy Liang
 */
public class Grammar
{
	...
	private boolean interpretBoolean(LispTree tree, Set<String> tags)
	{
		...
		{
			for (int i = 1; i < tree.children.size(); i++)
				if (!interpretBoolean(tree.child(i), tags))
					return false;
			return true;
		}
	...
	private void collectValidTagsBoolean(LispTree tree)
	{
		...
		if ("not".equals(command))
			...
			if ("and".equals(command) || "or".equals(command))
				for (int i = 1; i < tree.children.size(); i++)
	...
	private List<Rule> binarizeRule(Rule rule, List<Boolean> isOptionals)
	{
		...
		for (int i = 0; i < rule.rhs.size(); i++)
		{
			...
			{ // Whether to include newRhs.get(0)
				if (b0 == 0 && !newIsOptional.get(0))
					continue;
				for (int b1 = 0; b1 < 2; b1++)
				{ // Whether to include newRhs.get(1)
					...
					if (b0 == 1)
						rhs.add(newRhs.get(0));
					if (b1 == 1)
						rhs.add(newRhs.get(1));

					if (isRequiredCat.get(0) && isRequiredCat.get(1))
						sem = rule.sem;
					else
						if (b0 == 1 && Rule.isCat(rhs.get(0)) && isRequiredCat.get(1))
							sem = new SelectFn(1);
						else
							if (isRequiredCat.get(0) || isRequiredCat.get(1))
								sem = new SelectFn(0);
