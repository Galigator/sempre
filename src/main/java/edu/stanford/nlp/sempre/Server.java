	...
	class ExchangeState
	{
		...
		private HtmlElement makeCandidates(Example ex, String uri)
		{
			...
			for (int i = 0; i < ex.getPredDerivations().size(); i++)
			{
				...
				HtmlElement correct = makeTooltip(H.span().cls("correctButton").child("[Correct]"), H.div().cls("bubble").child("If this answer is correct, click to add as a new training example!"), uri + "&accept=" + i);
				String value = shorten(deriv.getValue() == null ? "" : deriv.getValue().toString(), 200);
				HtmlElement formula = makeTooltip(H.span(deriv.getFormula().toString()), H.div().cls("nowrap").child(makeDerivation(ex, deriv, false)), uri + "&select=" + i);
				HtmlElement row = H.tr().child(H.td(linkSelect(i, uri, i + " " + correct)).cls("nowrap")).child(H.td(Fmt.D(deriv.getScore()))).child(H.td(value)).style("width:250px");
				if (opts.htmlVerbose >= 1)
