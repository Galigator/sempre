...
public abstract class Formulas
{
	...
	public static boolean containsFreeVar(Formula formula, VariableFormula var)
	{
		...
		if (formula instanceof ReverseFormula) { return containsFreeVar(((ReverseFormula) formula).child, var); }
		if (formula instanceof AggregateFormula) { return containsFreeVar(((AggregateFormula) formula).child, var); }
		if (formula instanceof ArithmeticFormula) { return containsFreeVar(((ArithmeticFormula) formula).child1, var) || containsFreeVar(((ArithmeticFormula) formula).child2, var); }
	...
	public static String getBinaryId(Formula formula)
	{
		if (formula instanceof ReverseFormula)
		{
			...
			if (formula instanceof ValueFormula)
			{
				...
				if (v instanceof NameValue) { return ((NameValue) v).id; }
			}
		return null;
	}
