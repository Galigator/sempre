...
public abstract class ParserState
{
	...
	public enum CustomExpectedCount
	{
		NONE, UNIFORM, TOP, TOPALT, RANDOM,
	}
	...
	protected void featurizeAndScoreDerivation(final Derivation deriv)
	{
		...
		if (parser.verbose(5))
			LogInfo.logs("featurizeAndScoreDerivation(score=%s) %s %s: %s [rule: %s]", Fmt.D(deriv.score), deriv.cat, ex.spanString(deriv.start, deriv.end), deriv, deriv.rule);
		numOfFeaturizedDerivs++;
	}

	/**
	 * Prune down the number of derivations in |derivations| to the beam size. Sort the beam by score. Update beam statistics.
	 */
	protected void pruneCell(final String cellDescription, final List<Derivation> derivations)
	{
		if (derivations == null)
			return;
		...
		if (derivations.size() > maxCellSize)
		{
			...
			if (maxCellSize > 5000)
				LogInfo.logs("ParserState.pruneCell %s: maxCellSize = %s entries (not pruned yet)", maxCellDescription, maxCellSize);
		}
		...
		if (Parser.opts.verbose >= 3)
		{
			...
			for (final Derivation deriv : derivations)
				LogInfo.logs("%s(%s,%s): %s %s, [score=%s] allAnchored: %s", deriv.cat, deriv.start, deriv.end, deriv.formula, deriv.canonicalUtterance, deriv.score, deriv.allAnchored());
			LogInfo.end_track();
		}
		...
		if (ParserState.opts.pruneByProbDiff)
		{
			final double highestScore = derivations.get(0).score;
		...
		else
		{
			...
			final int beamSize = getBeamSize();
	...
	{
		final List<Derivation> derivs = new ArrayList<>();

		// All tokens (length 1)
		for (int i = 0; i < numTokens; i++)
		{
			derivs.add(new Derivation.Builder().cat(Rule.tokenCat).start(i).end(i + 1).rule(Rule.nullRule).children(Derivation.emptyList).withStringFormulaFrom(ex.token(i)).canonicalUtterance(ex.token(i)).createDerivation());

			// Lemmatized version
			derivs.add(new Derivation.Builder().cat(Rule.lemmaTokenCat).start(i).end(i + 1).rule(Rule.nullRule).children(Derivation.emptyList).withStringFormulaFrom(ex.lemmaToken(i)).canonicalUtterance(ex.token(i)).createDerivation());
		}
		...
		for (int i = 0; i < numTokens; i++)
			...
			{
				derivs.add(new Derivation.Builder().cat(Rule.phraseCat).start(i).end(j).rule(Rule.nullRule).children(Derivation.emptyList).withStringFormulaFrom(ex.phrase(i, j)).canonicalUtterance(ex.phrase(i, j)).createDerivation());

				// Lemmatized version
				derivs.add(new Derivation.Builder().cat(Rule.lemmaPhraseCat).start(i).end(j).rule(Rule.nullRule).children(Derivation.emptyList).withStringFormulaFrom(ex.lemmaPhrase(i, j)).canonicalUtterance(ex.phrase(i, j)).createDerivation());
			}
		return derivs;
	}
	...
	public void ensureExecuted()
	{
		...
		for (final Derivation deriv : predDerivations)
		{
			...
			if (!computeExpectedCounts && Parser.opts.executeTopFormulaOnly)
				break;
		}
		LogInfo.end_track();
	}
	...
	public static double compatibilityToReward(final double compatibility)
	{
	...
	/**
	 * Fill |counts| with the gradient with respect to the derivations according to a standard exponential family model over a finite set of derivations. Assume
	 * that everything has been executed, and compatibility has been computed.
	 */
	public static void computeExpectedCounts(final List<Derivation> derivations, final Map<String, Double> counts)
	{
		double[] trueScores;
		double[] predScores;

		final int n = derivations.size();
		if (n == 0)
			return;
		...
		if (opts.customExpectedCounts == CustomExpectedCount.TOP || opts.customExpectedCounts == CustomExpectedCount.TOPALT)
		{
			...
			if (opts.customExpectedCounts == CustomExpectedCount.RANDOM)
			{
		...
		for (int i = 0; i < n; i++)
		{
			final Derivation deriv = derivations.get(i);
			final double logReward = Math.log(compatibilityToReward(deriv.compatibility));
		...
		if (!NumUtils.expNormalize(trueScores))
			return;
		if (!NumUtils.expNormalize(predScores))
			return;
		...
		{
			final Derivation deriv = derivations.get(i);
			final double incr = trueScores[i] - predScores[i];
			if (incr == 0)
				continue;
	...
	private static int[] getTopDerivations(final List<Derivation> derivations)
	{
	...
	private static int[] getRandomDerivations(final List<Derivation> derivations)
	{
