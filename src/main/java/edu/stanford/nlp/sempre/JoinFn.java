...
/**
 * Input: two children (a binary and a unary in some order). JoinFn really serve two roles: 1. binary is just a relation (e.g.,
 * !fb:people.person.place_of_birth), in which case we do a join-project. 2. binary is a lambda calculus expression (e.g., (lambda x (count (var x)))), in which
 * case we do (macro) function application.
 *
 * @author Percy Liang
 */
public class JoinFn extends SemanticFn
{
	...
	/**
	 * There are four different ways binaries and unaries can be combined: - where was unary[Obama] binary[born]? (unary,binary unaryCanBeArg0) - unary[Spanish]
	 * binary[speaking] countries (unary,binary unaryCanBeArg1) - binary[parents] of unary[Obama] (binary,unary unaryCanBeArg0) - has binary[parents]
	 * unary[Obama] (binary,unary unaryCanBeArg1)
	 */
		...
		private Derivation doJoin(Derivation binaryDeriv, Formula binaryFormula, SemType binaryType, Derivation unaryDeriv, Formula unaryFormula, SemType unaryType, String featureDesc)
		{
			...
			if (!type.isValid())
			{
				if (opts.showTypeCheckFailures)
					LogInfo.warnings("JoinFn: type check failed: [%s : %s] JOIN [%s : %s]", binaryFormula, binaryType, unaryFormula, unaryType);
				return null;
			}
			...
			if (opts.typeInference)
			{
				...
				if (!fullType.isValid())
					return null; // Rule out logical form
