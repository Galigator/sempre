...
/**
 * Parse strings representing date ranges such as "20th century". The result has the form (and (< ...) (>= ...)). Currently only supports patterns like "1990's"
 * and "1800s"
 *
 * @author ppasupat
 */
	...
	public static class LazyDateRangeFnDerivs extends MultipleDerivationStream
	{
		...
		private void populateFormulas()
		{
			...
			while (year % range == 0)
			{
				...
				formulas.add(new MergeFormula(MergeFormula.Mode.and, new JoinFormula(new ValueFormula<Value>(new NameValue("<")), new ValueFormula<>(new NumberValue(year + range))), new JoinFormula(new ValueFormula<Value>(new NameValue(">=")), new ValueFormula<>(new NumberValue(year)))));
				range *= 10;
			}
		}
	}
