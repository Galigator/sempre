...
public final class BinaryLexicon
{
	...
	public List<BinaryLexicalEntry> buildEntry(LexiconValue lexValue, String lexemeKey)
	{
		...
		{
			BinaryLexicalEntry entry = new BinaryLexicalEntry(lexemeKey, lexemeKey, new HashSet<>(info.descriptions), lexValue.formula, source, info.popularity, info.expectedType1, info.expectedType2, info.unitId, info.unitDesc, alignmentScores, lexValue.lexeme);
	...
	public void updateLexicon(Pair<String, Formula> lexemeFormulaPair, int support)
	{
		...
		if (!exists)
		{
			...
			BinaryLexicalEntry newEntry = new BinaryLexicalEntry(lexeme, lexeme, new HashSet<>(bInfo.descriptions), bInfo.formula, EntrySource.FEEDBACK, bInfo.popularity, bInfo.expectedType1, bInfo.expectedType2, bInfo.unitId, bInfo.unitDesc, new HashMap<>(), lexeme);
	...
	public void sortLexiconByFeedback(Params params)
	{
		...
		LogInfo.log("Number of entries: " + lexemeToEntryList.size());
		BinaryLexEntrybyFeaturesComparator comparator = new BinaryLexEntrybyFeaturesComparator(params);
