...
/**
 * Executes all examples and reports any badness. Was used to prepare the Free917 dataset.
 *
 * @author Percy Liang
 */
public class ExecuteExamples implements Runnable
{
	...
	private boolean queryReturnsResults(Formula formula)
	{
		...
		if (!(response.value instanceof ListValue) || ((ListValue) response.value).values.size() == 0)
		{
