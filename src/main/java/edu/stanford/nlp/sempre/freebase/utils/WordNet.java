...
public final class WordNet
{
	...
	public enum PartOfSpeech
	{
		...
		public static PartOfSpeech fromWordNetSSType(char ssType)
		{
			...
			{
				if (pos.ssType == ssType) { return pos; }
			}
			if (ssType == 's') { return ADJECTIVE; }
	...
	public abstract class WordNetID implements Comparable<WordNetID>
	{
		...
		private Map<EdgeType, ArrayList<WordNetID>> links = new EnumMap<EdgeType, ArrayList<WordNetID>>(EdgeType.class);
	...
	public final class WordID extends WordNetID
	{
		...
		private final EnumMap<PartOfSpeech, WordTagID> mWordTags = new EnumMap<PartOfSpeech, WordTagID>(PartOfSpeech.class);
	...
	public final class WordTagID extends WordNetID
	{
		...
		private final ArrayList<SynsetID> mSynsets = new ArrayList<SynsetID>(1);
		...
		public SynsetID getSynset(int num)
		{
			...
			if (num < 0) { throw new IllegalArgumentException("SynsetIDs are 1-based"); }
	...
	public final class SynsetID extends WordNetID
	{
		...
		private final ArrayList<WordTagID> mWordTags = new ArrayList<WordTagID>(1);
		...
		{
			num--; // convert to 0-indexed
			if (num < 0) { throw new IllegalArgumentException("WordTagIDs are 1-based"); }
	...
	private static final Collection<WeakReference<WordNet>> instances = new LinkedList<WeakReference<WordNet>>();
	...
	public static WordNet load(File path)
	{
		...
		for (Iterator<WeakReference<WordNet>> it = instances.iterator(); it.hasNext();)
		{
			...
			if (wordnet == null)
			{
				...
				if (wordnet.path.equals(path)) { return wordnet; }
		}
	...
	public static WordNet loadPrologWordNet(File path)
	{
		...
		//
		{
			...
			for (Map.Entry<WordTagID, SynsetID> entry : deferredPositionSynsets.entrySet())
			{
				...
				if (!placed) { throw new AssertionError("Unable to place deferred synset"); }
			}
	...
	{
		if (loadedEdges.contains(type)) { throw new IllegalArgumentException("Unexpected error: trying to load " + type + " twice"); }
	...
	private void loadCLSRelations(File path)
	{
		...
		for (String line : ObjectBank.getLineIterator(new File(path, "wn_" + relation + ".pl")))
		{
			...
			if (fields.length != 5 || fields[4].length() != 1) { throw new IllegalArgumentException("Badly formed file for " + relation); }
	...
	private void addTranspose(EdgeType t1, EdgeType t2)
	{
		if (!loadedEdges.contains(t1))
		{
			...
			if (loadedEdges.contains(t2)) { throw new IllegalArgumentException("Cannot transpose: already contains " + t2); }
		loadedEdges.add(t2);
