		...
		if (arg2 instanceof ValueFormula && ((ValueFormula) arg2).value instanceof DateValue)
		{
			if (isOperator(relation))
			{
				if (relation.equals("="))
				{
					...
					return SparqlUtils.dateTimeStr(arg1) + " >= " + SparqlUtils.dateTimeStr(new ValueFormula<DateValue>(startDate)) + ") . FILTER (" + SparqlUtils.dateTimeStr(arg1) + " < " + SparqlUtils.dateTimeStr(new ValueFormula<DateValue>(endDate));
				}
				if (relation.equals("<="))
				{
