...
/**
 * Utilities for constructing SPARQL expressions. Note that we represent a subset of the SPARQL.
 * 
 * @author Percy Liang
 */
...
class SparqlBlock implements SparqlExpr
{
	...
	public SparqlBlock add(final SparqlExpr expr)
	{
		if (expr instanceof SparqlBlock)
			children.addAll(((SparqlBlock) expr).children);
		else
			children.add(expr);
		return this;
	}

	private String getId(final PrimitiveFormula formula)
	{
	...
	void addStatement(final PrimitiveFormula arg1, final String property, final PrimitiveFormula arg2, final boolean optional)
	{
	...
	public String toString()
	{
		...
		for (final SparqlExpr expr : children)
...
class SparqlSelect implements SparqlExpr
{
	static class Var
	{
		...
		public Var(final VariableFormula var, final String asValue, final String unit, final boolean isAuxiliary, final String description)
		{
			this.var = var;
	...
	public String toString()
	{
		...
		for (final Var var : selectVars)
			out.append(" " + var.toString());
		out.append(" WHERE " + where);
		if (sortVars.size() > 0)
		{
			out.append(" ORDER BY");
			for (final PrimitiveFormula sortVar : sortVars)
...
class SparqlUnion implements SparqlExpr
{
	...
	public SparqlUnion add(final SparqlBlock block)
	{
		children.add(block);
		return this;
	}
...
class SparqlNot implements SparqlExpr
{
	...
	public SparqlNot(final SparqlBlock block)
	{
		this.block = block;
	}
...
class SparqlStatement implements SparqlExpr
{
	...
	public SparqlStatement(final PrimitiveFormula arg1, final String relation, final PrimitiveFormula arg2, final boolean optional)
	{
		this.arg1 = arg1;
	...
	public SparqlStatement(final PrimitiveFormula arg1, final String relation, final PrimitiveFormula arg2, final boolean optional, final String options)
	{
		this(arg1, relation, arg2, optional);
		this.options = options;
	}

	public static boolean isIndependent(final String relation)
	{
		return relation.equals(":");
	}

	public static boolean isOperator(final String relation)
	{
		return relation.equals("=") || relation.equals("!=") || relation.equals("<") || relation.equals(">") || relation.equals("<=") || relation.equals(">=");
	}

	public static boolean isSpecialFunction(final String relation)
	{
		...
		if (arg2 instanceof ValueFormula && ((ValueFormula) arg2).value instanceof DateValue)
			if (isOperator(relation))
			{
				if (relation.equals("="))
				{
					...
					final DateValue startDate = (DateValue) ((ValueFormula) arg2).value;
					final DateValue endDate = advance(startDate);
					return SparqlUtils.dateTimeStr(arg1) + " >= " + SparqlUtils.dateTimeStr(new ValueFormula<>(startDate)) + ") . FILTER (" + SparqlUtils.dateTimeStr(arg1) + " < " + SparqlUtils.dateTimeStr(new ValueFormula<>(endDate));
				}
				if (relation.equals("<="))
				{
					...
					final DateValue startDate = (DateValue) ((ValueFormula) arg2).value;
					final DateValue endDate = advance(startDate);
					return SparqlUtils.dateTimeStr(arg1) + " < " + SparqlUtils.dateTimeStr(new ValueFormula<>(endDate));
				}
				if (relation.equals(">"))
				{
					// (> (date 2000 -1 -1)) really means >= (date 2001 -1 -1)
	...
	private DateValue advance(final DateValue date)
	{
...
final class SparqlUtils
{
	...
	public static String dateTimeStr(final PrimitiveFormula formula)
	{
		return "xsd:datetime(" + plainStr(formula) + ")";
	}

	public static String plainStr(final PrimitiveFormula formula)
	{
		...
		{
			final String s = ((StringValue) value).value;
