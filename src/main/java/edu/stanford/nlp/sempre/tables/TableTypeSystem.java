 ...
 * fb:cell_[fieldName].[string] | type = (union fb:type.cell fb:column.[fieldName]) PROPERTY: name = fb:row.row.[fieldName] | type = (-> (union fb:type.cell
 * fb:column.[fieldName]) fb:type.row) Note that the same string in different columns are mapped to different names.
 *
 * @author ppasupat
 */
