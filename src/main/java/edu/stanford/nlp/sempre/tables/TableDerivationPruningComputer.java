		...
		if (containsStrategy(forwardBackward) || containsStrategy(doubleNext) || containsStrategy(doubleCompares) || containsStrategy(emptyJoin))
		{
			...
			while (current instanceof JoinFormula)
			{
				...
				if (rid1 != null && rid2 != null)
				{
					...
					if (containsStrategy(doubleNext) && (rid1.equals(NEXT) || rid1.equals(PREV)) && (rid2.equals(NEXT) || rid2.equals(PREV)))
						return doubleNext;
					// doubleCompares: Prune (< (> ( ...)))
					if (containsStrategy(doubleCompares) && CanonicalNames.COMPARATORS.contains(rid1) && CanonicalNames.COMPARATORS.contains(rid2))
