...
/**
 * Represents a UnaryDenotation or a MappingDenotation. The following operations must be handled: - Map - count(UL) - Aggregate: sum(UL), ... - Combine - Merge:
 * and(UL1, UL2), ... - Arithmetic: sub(UL1, UL2), ... Compose operations (join, superlative) are handled in BL.
 *
 * @author ppasupat
 */
public interface Unarylike
{
	...
	/**
	 * Return a new Unarylike where only the values found in |upperBound| and domain values found in |domainUpperBound| are kept
	 */
