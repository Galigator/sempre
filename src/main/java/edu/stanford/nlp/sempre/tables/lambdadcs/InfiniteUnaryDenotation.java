...
public abstract class InfiniteUnaryDenotation extends UnaryDenotation
{
	...
	public static class RangeUnaryDenotation extends InfiniteUnaryDenotation
	{
		...
		public Value toValue()
		{
			return new InfiniteListValue(Arrays.asList(rangeEnds.leftComparator, rangeEnds.leftValue, rangeEnds.rightComparator, rangeEnds.rightValue));
		}
	...
	public static class GenericDateUnaryDenotation extends InfiniteUnaryDenotation
	{
		...
		public DateValue getMax()
		{
			if (date.day != -1)
				return date;
			if (date.month != -1)
				return new DateValue(date.year, date.month, YearMonth.of(date.year == -1 ? 2000 : date.year, date.month).lengthOfMonth());
			if (date.year != -1)
