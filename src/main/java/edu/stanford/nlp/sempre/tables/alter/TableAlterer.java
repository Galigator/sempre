...
public class TableAlterer
{
	...
	public TableKnowledgeGraph constructAlteredGraph(int alteredTableIndex)
	{
		...
		for (int i = 0; i < ex.numTokens(); i++)
		{
			...
			{
				for (Formula formula : oldGraph.getFuzzyMatchedFormulas(ex.getTokens(), i, j, FuzzyMatchFn.FuzzyMatchFnMode.ENTITY))
				{
	...
	private static final Comparator<TableCellProperties> NUMBER_COMP = new Comparator<TableCellProperties>()
	{
		...
		{
			Collection<Value> v1 = o1.metadata.get(TableTypeSystem.CELL_NUMBER_VALUE), v2 = o2.metadata.get(TableTypeSystem.CELL_NUMBER_VALUE);
			try
			{
	...
	private static final Comparator<TableCellProperties> DATE_COMP = new Comparator<TableCellProperties>()
	{
		...
		public int compare(TableCellProperties o1, TableCellProperties o2)
		{
			Collection<Value> v1 = o1.metadata.get(TableTypeSystem.CELL_DATE_VALUE), v2 = o2.metadata.get(TableTypeSystem.CELL_DATE_VALUE);
			try
			{
