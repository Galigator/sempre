 ...
 * format - context: the table used to answer the question - targetValue: the answer in its original format, possibly a `||`-separated list - tokens: the
 * question, tokenized - lemmaTokens: the question, tokenized and lemmatized - posTags: the part of speech tag of each token - nerTags: the name entity tag of
 * each token - nerValues: if the NER tag is numerical or temporal, the value of that NER span will be listed here - targetCanon: the answer, canonicalized -
...
public class TaggedDatasetGenerator extends TSVGenerator implements Runnable
{
	...
	private void dumpExample(final Example ex, final LispTree tree)
	{
		...
		if (targetValue instanceof NumberValue)
			...
			if (targetValue instanceof DateValue)
				fields[10] = "date";
			else
				fields[10] = "string";
		dump(fields);
