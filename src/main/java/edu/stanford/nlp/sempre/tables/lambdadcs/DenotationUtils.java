...
public final class DenotationUtils
{
	...
	public static UnaryDenotation arithmeticUnary(UnaryDenotation u1, UnaryDenotation u2, ArithmeticFormula.Mode mode, TypeProcessor processor)
	{
	 ...
	 */
	public static Unarylike superlative(int rank, int count, Unarylike head, Binarylike relation, SuperlativeFormula.Mode mode)
	{
		...
		if (head instanceof UnaryDenotation) { return superlativeUnary(rank, count, filtered.pairList.pairs, mode, processor); }
	 ...
	 */
	public static UnaryDenotation superlativeUnary(int rank, int count, List<Pair<Value, Value>> pairs, SuperlativeFormula.Mode mode, TypeProcessor processor)
	{
		...
		else
		{
			...
			int from = Math.min(rank - 1, indices.size()), to = Math.min(from + count, indices.size());
