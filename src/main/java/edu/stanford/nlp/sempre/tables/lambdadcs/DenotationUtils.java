...
/**
 * Utilities for denotations. Handle aggregation, superlative, and arithmetic operations on different types of values.
 *
 * @author ppasupat
 */
public final class DenotationUtils
{
	 ...
	 */
	public static int getSinglePositiveInteger(final UnaryDenotation unary)
	{
		if (unary.size() != 1)
			throw new LambdaDCSException(Type.nonSingletonList, "getSinglePositiveInteger(): denotation %s has != 1 elements", unary);
		final int amount = convertToInteger(unary.iterator().next());
		if (amount > 0)
			return amount;
	 ...
	 */
	public static double getSingleNumber(final UnaryDenotation unary)
	{
		if (unary.size() != 1)
	 ...
	 */
	public static Value getSingleValue(final UnaryDenotation unary)
	{
		if (unary.size() != 1)
	 ...
	 */
	public static Unarylike genericJoin(final Binarylike b, final Unarylike u)
	{
		if (u instanceof UnaryDenotation)
			return b.joinOnKey((UnaryDenotation) u);
		final Map<Value, Collection<Value>> mapping = new HashMap<>();
		if (u.domain().size() != Integer.MAX_VALUE)
		{
			for (final Value value : u.domain())
				mapping.put(value, b.joinOnKey(u.get(value)));
			return new MappingDenotation<>(u.getDomainVar(), new ExplicitPairList(mapping));
		}
		else
		{
			final ExplicitPairList binary = b.explicitlyFilterOnKey(u.range()).pairList;
			for (final Map.Entry<Value, UnaryDenotation> entry : binary.mapping.entrySet())
				for (final Value key : u.inverseGet(entry.getKey()))
	 ...
	 */
	public static Value aggregate(final Collection<Value> values, final AggregateFormula.Mode mode)
	{
		// Handle basic cases
		...
		// General cases
		final TypeProcessor processor = getTypeProcessor(values);
		switch (mode)
		{
			case max:
	...
	/**
	 * Helper: Check if the two Unarylikes have the same domain variable and return it. Throw an exception if the domain variables are not the same.
	 */
	public static String checkDomainVars(final Unarylike u1, final Unarylike u2)
	{
		if (u1 == null || u1.getDomainVar() == null)
			return u2.getDomainVar();
		if (u2 == null || u2.getDomainVar() == null)
			return u1.getDomainVar();
		if (u1.getDomainVar().equals(u2.getDomainVar()))
			return u1.getDomainVar();
		throw new LambdaDCSException(Type.invalidFormula, "Different domain variables: %s != %s", u1.getDomainVar(), u2.getDomainVar());
	}

	/**
	 * Merge values.
	 */
	public static Unarylike merge(final Unarylike u1, final Unarylike u2, final MergeFormula.Mode mode)
	{
		...
		final String domainVar = checkDomainVars(u1, u2);
		final Map<Value, UnaryDenotation> answer = new HashMap<>();
		for (final Value key : u1.domain())
			if (!answer.containsKey(key))
				answer.put(key, u1.get(key).merge(u2.get(key), mode));
		for (final Value key : u2.domain())
			if (!answer.containsKey(key))
				answer.put(key, u1.get(key).merge(u2.get(key), mode));
	...
	/**
	 * Perform arithmetic operation. Currently each Value must be a NumberValue. If arithmeticsFailOnMultipleElements is specified, throw an error if both
	 * children contain more than 1 Value.
	 */
	public static Unarylike arithmetic(final Unarylike u1, final Unarylike u2, final ArithmeticFormula.Mode mode)
	{
		final TypeProcessor processor = getTypeProcessor(u1.range(), u2.range());
		if (u1 instanceof UnaryDenotation && u2 instanceof UnaryDenotation)
	...
	public static UnaryDenotation arithmeticUnary(final UnaryDenotation u1, final UnaryDenotation u2, final ArithmeticFormula.Mode mode, TypeProcessor processor)
	{
		...
		if (processor == null)
			processor = getTypeProcessor(u1, u2);
		final List<Value> answer = new ArrayList<>();
		switch (mode)
		{
			case add:
				for (final Value v1 : u1)
					for (final Value v2 : u2)
						answer.add(processor.add(v1, v2));
				break;
			case sub:
				for (final Value v1 : u1)
					for (final Value v2 : u2)
						answer.add(processor.sub(v1, v2));
				break;
			case mul:
				for (final Value v1 : u1)
					for (final Value v2 : u2)
						answer.add(processor.mul(v1, v2));
				break;
			case div:
				for (final Value v1 : u1)
					for (final Value v2 : u2)
						answer.add(processor.div(v1, v2));
				break;
			default:
	...
	/**
	 * Compute the superlative (argmax/min rank count head (reverse relation)). Note that |relation| is reversed so that |relation| can be directly joined with
	 * |head|. If opt.aggregateReturnAllTopTies is true, (argmin 1 1 ...) and (argmax 1 1 ...) will return the list of all values that produce the min or max.
	 * Otherwise, only 1 arbitrary value will be returned.
	 */
	public static Unarylike superlative(final int rank, final int count, final Unarylike head, final Binarylike relation, final SuperlativeFormula.Mode mode)
	{
		// Filter the relation with the possible keys
		final BinaryDenotation<ExplicitPairList> filtered = relation.explicitlyFilterOnKey(head.range());
		final TypeProcessor processor = getTypeProcessor(filtered.pairList.range());
		...
		final Map<Value, UnaryDenotation> answer = new HashMap<>();
		for (final Value key : head.domain())
		{
			final BinaryDenotation<ExplicitPairList> refiltered = filtered.explicitlyFilterOnKey(head.get(key));
	 ...
	 */
	public static UnaryDenotation superlativeUnary(final int rank, final int count, final List<Pair<Value, Value>> pairs, final SuperlativeFormula.Mode mode, TypeProcessor processor)
	{
		...
		final List<Value> values = new ArrayList<>(), answer = new ArrayList<>();
		for (final Pair<Value, Value> pair : pairs)
			values.add(pair.getFirst());
		if (processor == null)
			processor = getTypeProcessor(values);
		...
		else
		{
			...
			final int from = Math.min(rank - 1, indices.size()), to = Math.min(from + count, indices.size());
			for (final int index : indices.subList(from, to))
	...
	public abstract static class TypeProcessor
	{
		...
		public Value max(final Collection<Value> values)
		{
		...
		public Value min(final Collection<Value> values)
		{
			if (!isSortable(values))
				throw new LambdaDCSException(Type.typeMismatch, "Values cannot be sorted.");
			Value min = null;
		...
		public List<Integer> argsort(final List<Value> values)
		{
			if (!isSortable(values))
				throw new LambdaDCSException(Type.typeMismatch, "Values cannot be sorted.");
			...
			for (int i = 0; i < values.size(); i++)
				indices.add(i);
			Collections.sort(indices, (o1, o2) -> compareValues(values.get(o1), values.get(o2)));
			return indices;
		}
	}
	...
	public static class NumberProcessor extends TypeProcessor
	{
		...
		@Override
		public boolean isCompatible(final Value v)
		{
			return v instanceof NumberValue;
		}

		@Override
		public boolean isSortable(final Collection<Value> values)
		{
			return true;
		}
	...
	public static class DateProcessor extends TypeProcessor
	{
		...
		public boolean isSortable(final Collection<Value> values)
		{
			...
			for (final Value value : values)
			{
				...
				{
					if (firstDate.year == -1 != (date.year == -1))
						return false;
					if (firstDate.month == -1 != (date.month == -1))
						return false;
					if (firstDate.day == -1 != (date.day == -1))
						return false;
				}
			}
			return true;
		...
		public int compareValues(final Value v1, final Value v2)
		{
			final DateValue d1 = (DateValue) v1, d2 = (DateValue) v2;
	 ...
	 */
	public static TypeProcessor getTypeProcessor(final Value value)
	{
	 ...
	 * Get the TypeProcessor corresponding to the strictest type.
	 */
	public static TypeProcessor getTypeProcessor(final Value value1, final Value value2)
	{
	 ...
	 * Get the TypeProcessor corresponding to the strictest type.
	 */
	public static TypeProcessor getTypeProcessor(final Collection<Value> values)
	{
	 ...
	 * Get the TypeProcessor corresponding to the strictest type.
	 */
	public static TypeProcessor getTypeProcessor(final Collection<Value> values1, final Collection<Value> values2)
	{
