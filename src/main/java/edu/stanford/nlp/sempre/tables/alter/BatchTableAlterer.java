...
public class BatchTableAlterer implements Runnable
{
	...
	public static enum SubsetChooserSpec
	{
	...
	public void run()
	{
		...
		switch (opts.subsetChooser)
		{
			...
			case ENTROPY:
				if (opts.numRetainedTables > 0)
					subsetChooser = new EntropySubsetChooser(opts.numAlteredTables, opts.numRetainedTables, opts.alsoTrySmallerSubsets);
				break;
			case PURE:
				if (opts.numRetainedTables > 0)
					subsetChooser = new PureSubsetChooser(opts.numAlteredTables, opts.numRetainedTables, opts.alsoTrySmallerSubsets);
				break;
			default: // Do nothing
		}
		...
		if (representativeDumper != null)
			representativeDumper.closeFile();
		if (retainedTablesOut != null)
			retainedTablesOut.close();
		if (turkInfoWriter != null)
			turkInfoWriter.close();
		if (turkMatchDumper != null)
